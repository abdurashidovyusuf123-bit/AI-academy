...
function selectSubject(id){
  ...
  document.getElementById('core-label').textContent = currentSubject.label;
  document.getElementById('chat-title').textContent = currentSubject.label + ' bo\'yicha AI ustoz';
  document.getElementById('chat-desc').textContent = currentSubject.desc;
...
async function sendMessage(){
  ...
  addMessage('user', text || '(rasm yuborildi)', imageToSend ? imageToSend.dataUrl : null);
...
// ---- Til tanlash (i18n) ----
let currentLang = localStorage.getItem('aiacademy_lang') || 'uz';
...
function buildCuteTheme(){
  clearThemeScene();
  themeScene.fog = new THREE.FogExp2(0x2a1f3a, 0.008);
  themeParticles = makeParticleField(1600, [0xff9dcf, 0xe0b3ff, 0xffffff, 0xffb8e0, 0xfff0a8], 65, 0.32);
  themeScene.add(themeParticles);

  const gemColors = [0xff8fc9, 0xd9a8ff, 0xffe0ef, 0xffd1ec];
  for(let i = 0; i < 6; i++){
    const geo = new THREE.IcosahedronGeometry(1.0 + Math.random()*0.8, 0);
    const color = gemColors[i % gemColors.length];
    const mat = new THREE.MeshStandardMaterial({ color, emissive: color, emissiveIntensity: 0.7, roughness: 0.3, metalness: 0.4 });
    const gem = new THREE.Mesh(geo, mat);
    gem.position.set((Math.random()-0.5)*24, (Math.random()-0.5)*15, -6 - Math.random()*14);
    themeScene.add(gem);
    themeExtras.push({ obj: gem, spin: 0.005 + Math.random()*0.005, floatSpeed: 0.4 + Math.random()*0.4, floatBase: gem.position.y });
  }
  const pinkLight = new THREE.PointLight(0xff8fc9, 1.2, 60);
  pinkLight.position.set(0, 5, 5);
  themeScene.add(pinkLight);
  themeScene.add(new THREE.AmbientLight(0x6b3868, 0.7));
}

function buildPirateTheme(){
  clearThemeScene();
  themeScene.fog = new THREE.FogExp2(0x0d1a1a, 0.016);
  themeParticles = makeParticleField(1400, [0xffb347, 0xffe08a, 0xffd27f, 0x9fe8d8], 62, 0.26);
  themeScene.add(themeParticles);

  const ship = new THREE.Group();
  const hullMat = new THREE.MeshStandardMaterial({ color: 0x5a3a20, roughness: 0.75, metalness: 0.1 });

  // Tanasi: bir nechta segmentdan tuzilgan, old qismi torayadigan korpus
  const hullShape = new THREE.Shape();
  hullShape.moveTo(-3.2, 0);
  hullShape.lineTo(2.6, 0);
  hullShape.quadraticCurveTo(3.6, 0, 3.6, -0.9);
  hullShape.lineTo(-2.8, -0.9);
  hullShape.quadraticCurveTo(-3.6, -0.5, -3.2, 0);
  const extrudeSettings = { depth: 1.8, bevelEnabled: true, bevelThickness: 0.15, bevelSize: 0.1, bevelSegments: 2 };
  ...
  const deckGeo = new THREE.BoxGeometry(6.2, 0.15, 1.7);
  const deckMat = new THREE.MeshStandardMaterial({ color: 0x7a5433, roughness: 0.85 });
  const deck = new THREE.Mesh(deckGeo, deckMat);
  deck.position.y = 0.55;
  ship.add(deck);

  [[-1.4, 3.2], [1.6, 4.4]].forEach(([x, h]) => {
    const mastMat = new THREE.MeshStandardMaterial({ color: 0x2e1c10 });
    const mast = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.1, h, 8), mastMat);
    mast.position.set(x, 0.6 + h/2, 0);
    ship.add(mast);
    const sailMat = new THREE.MeshStandardMaterial({ color: 0xf0e6cc, side: THREE.DoubleSide, roughness: 0.85 });
    const sail = new THREE.Mesh(new THREE.PlaneGeometry(1.7, h * 0.62), sailMat);
    sail.position.set(x, 0.9 + h * 0.5, 0.12);
    ship.add(sail);
  });

  const flagMat = new THREE.MeshStandardMaterial({ color: 0xd94d4d, emissive: 0xd94d4d, emissiveIntensity: 0.3, side: THREE.DoubleSide });
  const flag = new THREE.Mesh(new THREE.PlaneGeometry(0.7, 0.4), flagMat);
  flag.position.set(1.6, 5.4, 0.1);
  ship.add(flag);

  const lanternMat = new THREE.MeshStandardMaterial({ color: 0xffcf7a, emissive: 0xffb347, emissiveIntensity: 1.4 });
  const lantern = new THREE.Mesh(new THREE.SphereGeometry(0.16, 10, 10), lanternMat);
  lantern.position.set(-3, 1.1, 0.9);
  ship.add(lantern);
  const lanternLight = new THREE.PointLight(0xffb347, 1.6, 12);
  lanternLight.position.copy(lantern.position);
  ship.add(lanternLight);

  ship.position.set(-1, -5, -13);
  ship.rotation.y = 0.35;
  ship.scale.setScalar(1.15);
  themeScene.add(ship);
  themeExtras.push({ obj: ship, rock: true, floatBase: ship.position.y });

  const moon = new THREE.PointLight(0x9fd8ff, 0.9, 60);
  moon.position.set(6, 12, -4);
  themeScene.add(moon);
  themeScene.add(new THREE.AmbientLight(0x223333, 0.6));
}

function buildNatureTheme(){
  clearThemeScene();
  themeScene.fog = new THREE.FogExp2(0x9fd0e0, 0.012);
  themeParticles = makeParticleField(800, [0xffffff, 0xd8f0ff, 0x9dffc0], 58, 0.22);
  themeScene.add(themeParticles);

  const mountainColors = [0x2e5040, 0x3f6a4f, 0x578566];
  for(let i = 0; i < 6; i++){
    const h = 7 + Math.random()*7;
    const geo = new THREE.ConeGeometry(3.2 + Math.random()*2, h, 5);
    const mat = new THREE.MeshStandardMaterial({ color: mountainColors[i % mountainColors.length], flatShading: true, roughness: 1 });
    const mtn = new THREE.Mesh(geo, mat);
    mtn.position.set((i - 2.5) * 5 + (Math.random()-0.5)*2, h/2 - 9, -22 - Math.random()*8);
    mtn.rotation.y = Math.random() * Math.PI;
    themeScene.add(mtn);
  }

  const treeColors = [0x2f7a4a, 0x3f9158, 0x5cb06e];
  for(let i = 0; i < 14; i++){
    ...
    const leafMat = new THREE.MeshStandardMaterial({ color: leafColor, emissive: leafColor, emissiveIntensity: 0.25, flatShading: true, roughness: 0.8 });
    for(let j = 0; j < 3; j++){
      const cone = new THREE.Mesh(new THREE.ConeGeometry(0.9 - j*0.2, 1.3, 7), leafMat);
      cone.position.y = 1.5 + j * 0.85;
      tree.add(cone);
    }
    const scale = 0.7 + Math.random() * 0.9;
    tree.scale.setScalar(scale);
    tree.position.set((Math.random()-0.5)*26, -8.5, -6 - Math.random()*10);
    themeScene.add(tree);
  }
  ...
  for(let i = 0; i < fallCount; i++){
    fallPos[i*3] = 8 + (Math.random()-0.5)*0.7;
    fallPos[i*3+1] = Math.random()*16 - 7;
    fallPos[i*3+2] = -17;
  }
  fallGeo.setAttribute('position', new THREE.BufferAttribute(fallPos, 3));
  const fallMat = new THREE.PointsMaterial({ color: 0xcaf0ff, size: 0.16, transparent: true, opacity: 0.9, blending: THREE.AdditiveBlending });
  const waterfall = new THREE.Points(fallGeo, fallMat);
  themeScene.add(waterfall);
  themeExtras.push({ obj: waterfall, isWaterfall: true });

  const sun = new THREE.DirectionalLight(0xffffff, 1.3);
  sun.position.set(5, 10, 5);
  themeScene.add(sun);
  const ambient = new THREE.AmbientLight(0xaee0ff, 0.7);
  themeScene.add(ambient);
}
...
function buildDefaultTheme(){
  clearThemeScene();
  themeScene.fog = new THREE.FogExp2(0x0b0e1a, 0.012);
  themeParticles = makeParticleField(500, [0x5EEAD4, 0x7DA6FF, 0xC77DFF], 60, 0.14);
  themeScene.add(themeParticles);
}

const THEME_BUILDERS = {
  space: buildSpaceTheme, cute: buildCuteTheme, pirate: buildPirateTheme,
  nature: buildNatureTheme, tech: buildTechTheme, default: buildDefaultTheme
};
...
const THEME_COLORS = {
  space: { bg:'#05060f', surface:'#12142a', surface2:'#1c2148', line:'#33397a', text:'#f0f2ff', accent:'#7fd4ff', overlay:'rgba(5,6,20,0.28)' },
  cute: { bg:'#3a1f42', surface:'#54295f', surface2:'#6f3676', line:'#8a4d8f', text:'#fff2fb', accent:'#ff6fb8', overlay:'rgba(58,20,55,0.28)' },
  pirate: { bg:'#0a1a1a', surface:'#153030', surface2:'#1e4040', line:'#2f5c56', text:'#fdf1d9', accent:'#ffb347', overlay:'rgba(6,20,18,0.3)' },
  nature: { bg:'#123024', surface:'#1c4a35', surface2:'#276147', line:'#3d8a63', text:'#eefff2', accent:'#7ee08c', overlay:'rgba(10,32,24,0.25)' },
  tech: { bg:'#170505', surface:'#2c0c0c', surface2:'#421212', line:'#6b1e1e', text:'#fff0ea', accent:'#ff5c3d', overlay:'rgba(20,5,5,0.3)' },
};
...
function applyThemeColors(themeKey){
  ...
  root.setProperty('--surface-2', c.surface2);
  root.setProperty('--line', c.line);
  root.setProperty('--text', c.text);
  root.setProperty('--accent', c.accent);
  document.getElementById('theme-canvas').style.filter = 'saturate(1.35) brightness(1.15)';
